...
bp_db = Blueprint('blueprint_db', __name__)

db = Database()

@bp_db.route("/mdb/update_diff", methods=["POST"])
def update_diff():
    global db
    try:
...
@bp_db.route("/mdb/update_db", methods=["POST"])
def update_db():
    global db
    try:
...
@bp_db.route("/mdb/update_chapter", methods=["POST"])
def update_chapter():
    global db
    try:
        db.reset_logs()
...
@bp_db.route("/mdb/db_overview", methods=["GET"])
def db_overview():
    global db
    try:
