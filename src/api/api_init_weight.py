...
from src.modules.init_weight import InitWeight

bp_init_weight = Blueprint('blue_prin_iw', __name__)

cur_chapter = "chuong-1"
init_weight_agent = InitWeight(cur_chapter="chuong-1")

@bp_init_weight.route("/init_weight", methods=["POST"])
def history_train():
...
@bp_init_weight.route("/history_train/score_plot/<cur_chapter>", methods=["GET"])
